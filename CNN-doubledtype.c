...
double apply_conv_test(Image_t image, int** labels,  int classes,int f,int conv_s, Filter_t filt1 ,Matrix_t bias1, int pool_f,int pool_s,Filter_t filt2,Matrix_t bias2,Matrix_t w3,Matrix_t w4,Matrix_t b3,Matrix_t b4,int* predicted,Filter_t filt5,Matrix_t bias5,Filter_t filt7,Matrix_t bias7){
...
void adam_filter(Filter_t f,Filter_t v,Filter_t s, Filter_t d, double beta1, double beta2, double lr,int debugger,int bias_correction_term){
  Filter_t d_copy_2;
...
void adam_matrix(Matrix_t w,Matrix_t v,Matrix_t s, Matrix_t d, double beta1, double beta2, double lr,int debugger, int bias_correction_term){
  Matrix_t d_copy;
...
double optimizer(Image_t* batch_images,int batch, int classes, double lr,double beta1, double beta2, Filter_t* filters,Matrix_t* weights,Matrix_t* biases, Filter_t* filters_grad,Matrix_t* weights_grad,Matrix_t* biases_grad, int itr,int* labels,int conv_s, int pool_s,int f,int pool_f,Filter_t* dfilters,Matrix_t* dbiases,Matrix_t* dweights,int* correct, int bias_correction_term){
  ...
  double loss=apply_conv(batch_images[i], y, classes,f,conv_s,filters[0],biases[0],pool_f,pool_s,filters[1],biases[1],weights[0],weights[1],grad_w,grad_f,grad_b,biases[2],biases[3],copy_df1,copy_df2,copy_db1,copy_db2,copy_db3,copy_db4,copy_dw3,copy_dw4,&predicted,filters[2],biases[4],copy_df5,copy_db5,filters[3],biases[5],copy_df7,copy_db7);
...
double optimizer_test(Image_t* batch_images,int batch, int classes, double lr,double beta1, double beta2, Filter_t* filters,Matrix_t* weights,Matrix_t* biases, int* labels,int conv_s, int pool_s,int f,int pool_f,int* correct){
  double cost=0;
  // initialize gradients and momentum,RMS params
  ...
  int predicted;
  double loss= apply_conv_test(batch_images[i], y, classes,f,conv_s,filters[0],biases[0],pool_f,pool_s,filters[1],biases[1],weights[0],weights[1],biases[2],biases[3],&predicted,filters[2],biases[4],filters[3],biases[5]);
...
int main( int argc, char** argv ) {
  ...
  if(test==0){  
    ...
    */
    ...
    //fill_matrix(b1,"weights_1930/b1_trained_aug_v13");
    Filter_t dfilt1;
    ...
    //fill_matrix(b2,"weights_1930/b2_trained_aug_v13");
    Filter_t dfilt2;
    ...
    //fill_matrix(b5,"weights_1930/b5_trained_aug_v13");
    Filter_t dfilt5;
    ...
    //fill_matrix(b7,"weights_1930/b7_trained_aug_v13");
    Filter_t dfilt7;
    ...
    //fill_matrix(b3,"weights_1930/b3_trained_aug_v13");
    ...
    //fill_matrix(b4,"weights_1930/b4_trained_aug_v13");
    ...
    */
    ...
    for(int i=0;i<epochs;i++){
      ...
      for(int j=0;j<batch_iterations;j++){
        ...
        int correct=0;
        double loss_tot =optimizer(batch_images,batch, classes, lr,beta1, beta2, filters,weights,biases, filters_grad,weights_grad,biases_grad,itr,labels, conv_s, pool_s,f,pool_f,dfilters,dbiases,dweights,&correct,bias_correction_term);
    ...
    char w3_file[33];
    char w4_file[33];
    snprintf(f1_file, 33, "weights_1930/f1_trained_aug_v%d", file_update);
    snprintf(f2_file, 33, "weights_1930/f2_trained_aug_v%d", file_update);
    snprintf(f5_file, 33, "weights_1930/f5_trained_aug_v%d", file_update);
    snprintf(f7_file, 33, "weights_1930/f7_trained_aug_v%d", file_update);
    snprintf(b1_file, 33, "weights_1930/b1_trained_aug_v%d", file_update);
    snprintf(b2_file, 33, "weights_1930/b2_trained_aug_v%d", file_update);
    snprintf(b3_file, 33, "weights_1930/b3_trained_aug_v%d", file_update);
    snprintf(b4_file, 33, "weights_1930/b4_trained_aug_v%d", file_update);
    snprintf(b5_file, 33, "weights_1930/b5_trained_aug_v%d", file_update);
    snprintf(b7_file, 33, "weights_1930/b7_trained_aug_v%d", file_update);
    snprintf(w3_file, 33, "weights_1930/w3_trained_aug_v%d", file_update);
    snprintf(w4_file, 33, "weights_1930/w4_trained_aug_v%d", file_update);
 ...
 for(int t=0;t<50;t++){ 
    ...
    */
    ...
    char f1_file[33];
    snprintf(f1_file, 33, "weights_1930/f1_trained_aug_v%d", t);
    Filter_t f1;
    initializeFilter(&f1,32,image_channels,f);
    fill_filter(f1, f1_file);

    char b1_file[33];
    snprintf(b1_file, 33, "weights_1930/b1_trained_aug_v%d", t);
    Matrix_t b1;
    matrix_initialize_n(&b1,32,1);
    ...
    char f2_file[33];
    snprintf(f2_file, 33, "weights_1930/f2_trained_aug_v%d", t);
    Filter_t f2;
    initializeFilter(&f2,128,128,f);
    fill_filter(f2, f2_file);
    
    char b2_file[33];
    snprintf(b2_file, 33, "weights_1930/b2_trained_aug_v%d", t);
    Matrix_t b2;
    matrix_initialize_n(&b2,128,1);
    fill_matrix(b2,b2_file);
    
    char f5_file[33];
    snprintf(f5_file, 33, "weights_1930/f5_trained_aug_v%d", t);
    Filter_t f5;
    initializeFilter(&f5,64,32,f);
    fill_filter(f5, f5_file);
    
    char b5_file[33];
    snprintf(b5_file, 33, "weights_1930/b5_trained_aug_v%d", t);
    ...
    char f7_file[33];
    snprintf(f7_file, 33, "weights_1930/f7_trained_aug_v%d", t);
    ...
    char b7_file[33];
    snprintf(b7_file, 33, "weights_1930/b7_trained_aug_v%d", t);
    ...
    char w3_file[33];
    snprintf(w3_file, 33, "weights_1930/w3_trained_aug_v%d", t);
    ...
    char w4_file[33];
    snprintf(w4_file, 33, "weights_1930/w4_trained_aug_v%d", t);
    Matrix_t w4;
    matrix_initialize_n(&w4,2,512);
    ...
    char b3_file[33];
    snprintf(b3_file, 33, "weights_1930/b3_trained_aug_v%d", t);
    Matrix_t b3;
    matrix_initialize_n(&b3,512,1);
    ...
    char b4_file[33];
    snprintf(b4_file, 33, "weights_1930/b4_trained_aug_v%d", t);
    Matrix_t b4;
    matrix_initialize_n(&b4,2,1);
    ...
    filters[3]=f7;
      ...
      */
      ...
      int correct=0;
      double loss_tot =optimizer_test(batch_images,batch, classes, lr,beta1, beta2, filters,weights,biases, labels, conv_s, pool_s,f,pool_f,&correct);
      correct_total+=correct;
      printf("%s %f\n","Acc on test",(double)correct_total/(tot_img_test_cat+tot_img_test_dog));
...
// gcc -o arq1930 arquitectura1930.c -lm -O3
